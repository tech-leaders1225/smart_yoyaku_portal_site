...
RSpec.describe "StoreManager::Plans", type: :request do
  ...
  #       sign_in @store_manager
  #       post store_manager_plans_path(@plan)
  #       expect(response.status).to eq 200
  #     end
  #     it "入力内容が不正な場合、planが新規作成されないこと" do
  #       @plan = @plan.update( plan_name: "文字列オーバーです"*30)
  #       sign_in @store_manager
  #       post store_manager_plans_path(@plan)
  #       expect(@plan.reload.plan_name).to_not eq "文字列オーバーです"
  #       expect(response.status).to eq 302
  #     end
  #   end
  # end
